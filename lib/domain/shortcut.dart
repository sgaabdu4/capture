...
class Shortcut {
  ...
  factory Shortcut.fromJson(Map<String, Object?> json) => Shortcut(
    json['key']! as String,
    modifiers: {
      for (final m in json['modifiers']! as List<Object?>)
        Modifier.values.byName(m! as String),
    },
  );
  ...
  static const defaultShortcut = Shortcut(
    'R',
    modifiers: {Modifier.control, Modifier.option},
  );

  int get keyCode => carbonKeyCodes[key]!;

  int get carbonModifiers =>
      modifiers.fold(0, (flags, m) => flags | m.carbonFlag);
...
String? shortcutProblem(Shortcut s) {
  ...
  if (s.modifiers.length < 2 ||
      !(s.modifiers.contains(Modifier.control) ||
          s.modifiers.contains(Modifier.command))) {
