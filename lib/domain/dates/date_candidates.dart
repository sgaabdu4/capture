...
final _dayPatterns = <(RegExp, DayCandidate Function(RegExpMatch, String, SourceSpan))>[
  (
    _re(r'\b(the )?day after tomorrow\b'),
    (m, id, s) => DayCandidate(id, s, DayKind.dayAfter),
  ),
  (
  ...
  (
    _re(
      '\\b(\\d{1,2})(?:st|nd|rd|th)?(?: of)? ($_monthNames)\\b(?:,? (\\d{4}))?',
    ),
    (m, id, s) => DayCandidate(
      id,
      s,
  ...
  (
    _re(
      '\\b($_monthNames) (?:the )?(\\d{1,2})(?:st|nd|rd|th)?\\b(?:,? (\\d{4}))?',
    ),
    (m, id, s) => DayCandidate(
      id,
      s,
...
final _periodAfter = _re(
  r'^\s*(?:in the (morning|afternoon|evening)|at night|tonight)',
);

final _timePatterns =
    <(RegExp, TimeCandidate Function(RegExpMatch, String, SourceSpan))>[
      (
      ...
      (
        _re('\\b(\\d{1,2}) ?$_ampm(?=\\W|\$)'),
        (m, id, s) => _exact(id, s, int.parse(m[1]!), 0, m[2]!),
      ),
      (
        _re(
          '\\b($_hourWords)(?: (fifteen|thirty|forty-five))? ?$_ampm(?=\\W|\$)',
        ),
        (m, id, s) => _exact(
          id,
          s,
      ...
      (
        _re(r'\b(noon|midday)\b'),
        (m, id, s) => TimeCandidate(id, s, TimeKind.exact, hour: 12),
      ),
      (
        _re(r'\bmidnight\b'),
        (m, id, s) => TimeCandidate(id, s, TimeKind.exact, hour: 0),
      ),
      (
      ...
      (
        ...
        (m, id, s) =>
            TimeCandidate(id, s, TimeKind.ambiguous, hour: _quantity(m[1]!)),
      ),
      (
        _re(
          '\\bat (\\d{1,2}|$_hourWords)(?: (fifteen|thirty|forty-five))?\\b(?![:./]\\d)',
        ),
      ...
      (
        ...
        (m, id, s) => TimeCandidate(
          ...
          s,
          TimeKind.inMinutes,
          minutes: _durationMinutes(m[1]!, m[3]!),
        ),
      ),
      (
...
FoundCandidates findDateCandidates(
  String transcript,
  SourceSpan span, {
  String prefix = '',
}) {
    ...
    return [
      for (var i = 0; i < found.length; i++)
        found[i].$2(
          ...
          SourceSpan.of(
            transcript,
            span.start + found[i].$1,
            span.start + found[i].$3,
          ),
        ),
    ];
  }
  ...
  return FoundCandidates(days, [
    for (final t in times) _withPeriodAfter(t, transcript),
  ]);
}
...
int _quantity(String text) =>
    int.tryParse(text) ?? _numbers[text.toLowerCase()] ?? 1;
