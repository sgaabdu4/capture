...
class LocalStore {
  ...
  List<CaptureRecord> captures() => [
    for (final row in _db.select('SELECT json FROM captures ORDER BY captured_at DESC'))
      _decode(row),
  ];

  void deleteCapture(String id) => _db.execute('DELETE FROM captures WHERE id = ?', [id]);

  CaptureRecord _decode(Row row) =>
      CaptureRecord.fromJson(jsonDecode(row['json'] as String) as Map<String, Object?>);
  ...
  List<Group> groups() {
    ...
    return [
      for (final g in jsonDecode(raw) as List<Object?>) Group.fromJson(g! as Map<String, Object?>),
    ];
  }
  ...
  List<LibraryEntry> library() => [
    for (final row in _db.select('SELECT json FROM library'))
      LibraryEntry.fromJson(jsonDecode(row['json'] as String) as Map<String, Object?>),
  ];
