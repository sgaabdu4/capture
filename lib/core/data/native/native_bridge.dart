...
class NativeBridge implements NativeApi {
  ...
  @override
  Future<bool> requestMic() async => await _channel.invokeMethod<bool>('requestMic') ?? false;

  @override
  Future<void> startRecording(String path, {double maxSeconds = 300}) =>
      _channel.invokeMethod('startRecording', {'path': path, 'maxSeconds': maxSeconds});

  @override
  Future<RecordingResult?> stopRecording() async {
    final result = await _channel.invokeMapMethod<String, Object?>('stopRecording');
    if (result == null) return null;
    return RecordingResult(result['path']! as String, (result['seconds']! as num).toDouble());
  }
  ...
  Future<void> showReview({
    ...
    'rows': [
      for (final r in rows) {'id': r.id, 'icon': r.icon, 'title': r.title, 'detail': r.detail},
    ],
  });
  ...
  @override
  Future<int> encodeM4a(String input, String output, {int bitRate = 32000}) async =>
