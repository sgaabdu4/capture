...
class NotionWorkspace {
  ...
  factory NotionWorkspace.fromJson(Map<String, Object?> json) => NotionWorkspace(
...
class NotionSetup {
  ...
  Future<NotionWorkspace> connect(String parentPageId, {NotionWorkspace? known}) async {
    ...
    if (known != null && known.parentPageId == parentPageId && await _stillThere(known)) {
      return build(known.areaPageId, [known.groups, known.captures, known.library]);
    }
    final area = await _findArea(parentPageId) ?? await _createArea(parentPageId);
  ...
  Future<String?> _findArea(String parentPageId) async {
    for (final child in await _children(parentPageId)) {
      ...
      final title = (child['child_page'] as Map<String, Object?>?)?['title'] as String?;
  ...
  Future<Map<String, String>> _findDataSources(String areaId) async {
    ...
    for (final child in await _children(areaId)) {
      ...
      final title = (child['child_database'] as Map<String, Object?>?)?['title'] as String?;
      if (title == null) continue;
      final db = await _client.get('/v1/databases/${child['id']}');
      final sources = db['data_sources'] as List<Object?>? ?? const [];
      if (sources.isEmpty) continue;
      byTitle[title] = (sources.first! as Map<String, Object?>)['id']! as String;
    }
    return byTitle;
  }
  ...
  Future<List<String>> _ensureDataSources(String areaId) async {
    ...
    final groups = found['Groups'] ?? await _database(areaId, 'Groups', _groupsSchema);
    final captures = found['Captures'] ?? await _database(areaId, 'Captures', _capturesSchema);
    final library =
        found['Library'] ?? await _database(areaId, 'Library', _librarySchema(groups, captures));
  ...
  Future<String> _database(String areaId, String title, Map<String, Object?> properties) async {
  ...
  Future<Group> createGroup(String dataSource, String name, String description) async {
    ...
    return Group(id: page['id']! as String, name: name, description: description);
  }

  Future<void> updateGroup(Group group) => _client.patch('/v1/pages/${group.id}', {
    'properties': {
