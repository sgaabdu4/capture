...
Future<void> _downloadMain(List<Object?> args) async {
  ...
  try {
    ...
    for (final f in files) {
      await _fetch(client, base, dir, f, (n) => send.send(['progress', done + n, total]));
      done += f.bytes;
      send.send(['verifying', f.name]);
      await _verify(dir, f);
    }
    File('$dir/verified.json').writeAsStringSync(
      jsonEncode({'revision': parakeetRevision, for (final f in files) f.name: f.sha256}),
    );
    send.send(['ready']);
  } on Object catch (e) {
...
Future<void> _verify(String dir, ModelFile f) async {
  ...
  if (digest.toString() != f.sha256) {
    file.deleteSync();
    throw StateError('${f.name} failed its checksum and was deleted. Retry the download.');
  }
}
