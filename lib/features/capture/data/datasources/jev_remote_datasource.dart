...
class JevException implements Exception {
  ...
  String get userMessage => switch (failure) {
    ...
    JevFailure.rateLimited => 'TypeSafe is rate limiting requests. Try again shortly.',
...
class JevClient {
  ...
  Future<(JevResult, JevCallMetrics)> ask(String state, Map<String, JevQuestion> questions) async {
  ...
  bool _retryable(int status) => status == 408 || status == 429 || status >= 500;
