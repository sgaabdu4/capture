...
class NotionSaver {
  ...
  Future<String?> _findBy(String dataSource, String property, String value) async {
    ...
    return pages.isEmpty ? null : (pages.first! as Map<String, Object?>)['id']! as String;
  }
  ...
  Future<CaptureRecord> _capturePage(CaptureRecord r) async {
    final id =
        ...
        (await _client.post('/v1/pages', {
              'parent': {'type': 'data_source_id', 'data_source_id': _workspace.captures},
              'properties': {
  ...
  Future<CaptureRecord> _item(CaptureRecord r, ProposalItem item) async {
    final id =
        ...
        (await _client.post('/v1/pages', {
              'parent': {'type': 'data_source_id', 'data_source_id': _workspace.library},
              'properties': itemProperties(r, item),
              'children': itemBlocks(item),
            }))['id']!
            as String;
    return r.copyWith(
      progress: r.progress.copyWith(itemPages: {...r.progress.itemPages, item.id: id}),
    );
  }
  ...
  Future<CaptureRecord> _audio(CaptureRecord r) async {
    ...
    final upload = await _client.uploadFile(bytes, filename: name, contentType: 'audio/mp4');
    ...
    return r.copyWith(progress: r.progress.copyWith(audioUploadId: upload, audioAttached: true));
  }
  ...
  Map<String, Object?> itemProperties(CaptureRecord r, ProposalItem item) => {
    ...
    P.reminder: dateValue(item.kind == ItemKind.task ? item.reminder : null, r.timeZone),
  };
}
