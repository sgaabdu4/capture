...
ProposalItem setDue(ProposalItem item, DueDate? due, {Set<ReviewFlag> flags = const {}}) =>
    item.copyWith(
      due: () => due,
...
(ProposalItem, ProposalItem)? splitItem(
  ...
  final left = [...item.sources.take(index), SourceSpan.of(transcript, span.start, leftEnd)];
  final right = [SourceSpan.of(transcript, rightStart, span.end), ...item.sources.skip(index + 1)];
...
ProposalItem mergeItems(ProposalItem first, ProposalItem second) {
  final sources = [...first.sources, ...second.sources]..sort((a, b) => a.start.compareTo(b.start));
  final excerpt = sources.map((s) => s.excerpt).join(' ');
  final bodyEdited = first.edited.contains('body') || second.edited.contains('body');
  final conflicting = first.groupId != second.groupId || first.kind != second.kind;
  return first.copyWith(
    sources: sources,
    body: bodyEdited ? '${first.body}\n${second.body}' : proposedBody(excerpt),
    title: first.edited.contains('title') ? first.title : proposedTitle(excerpt),
