...
class Group {
  ...
  bool get isUnsorted => name.trim().toLowerCase() == unsortedName.toLowerCase();
...
const defaultGroups = [
  ...
  (name: unsortedName, description: 'None of the other groups fits, or the user needs to choose.'),
];
...
String? validateGroup(
  ...
  final duplicate = existing.any(
    (g) => g.id != editingId && g.name.trim().toLowerCase() == trimmed.toLowerCase(),
  );
...
class DueDate {
  ...
  DueDate withTime(int hour, int minute) => DueDate(year, month, day, hour: hour, minute: minute);
  ...
  /// `2026-09-19T14:00:00`.
  String get iso {
    final date = '${year.toString().padLeft(4, '0')}-${_two(month)}-${_two(day)}';
...
class ProposalItem {
  ...
  factory ProposalItem.fromJson(Map<String, Object?> json) => ProposalItem(
    ...
    due: json['due'] == null ? null : DueDate.fromJson(json['due']! as Map<String, Object?>),
    ...
    edited: {for (final f in json['edited'] as List<Object?>? ?? const []) f! as String},
...
String proposalSummary(Iterable<ProposalItem> items) {
  ...
  final reminders = included.where((i) => i.kind == ItemKind.task && i.reminder != null).length;
