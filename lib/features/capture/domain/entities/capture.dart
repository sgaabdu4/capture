...
class SaveProgress {
  ...
  factory SaveProgress.fromJson(Map<String, Object?> json) => SaveProgress(
    ...
    itemPages: {
      for (final e in (json['itemPages'] as Map<String, Object?>? ?? const {}).entries)
    ...
    remindersScheduled: {
      for (final id in json['remindersScheduled'] as List<Object?>? ?? const []) id! as String,
    },
  );
...
class CaptureRecord {
  ...
  bool get isOpen => stage != CaptureStage.saved && stage != CaptureStage.dismissed;
