...
class _Splitter {
  _Splitter(this.text) : words = _word.allMatches(text).toList(growable: false) {
  ...
  bool _isAbbreviation(RegExpMatch end) {
    ...
    final before = words.lastWhere((w) => w.end <= end.start, orElse: () => words.first);
  ...
  List<TranscriptUnit> units() {
    ...
    for (final cut in [...starts, text.length]) {
      ...
      if (end > unitStart) {
        ...
        units.add(TranscriptUnit(id, SourceSpan.of(text, unitStart, end), kind));
      }
      unitStart = cut;
