...
ResolvedDate _inMinutes(tz.TZDateTime captured, TimeCandidate time, {required bool hasDay}) {
...
DueDate? _resolveDay(tz.TZDateTime captured, DayCandidate day, Set<ReviewFlag> flags) {
...
DueDate? _calendar(DateTime today, int? year, int month, int dayOfMonth, Set<ReviewFlag> flags) {
  ...
  return candidate == null ? null : DueDate(candidate.year, candidate.month, candidate.day);
}
...
DueDate _nextOccurrence(tz.TZDateTime captured, int hour, int minute, Set<ReviewFlag> flags) {
  final laterToday = hour > captured.hour || (hour == captured.hour && minute > captured.minute);
  ...
  flags.add(ReviewFlag.checkDate);
  final t = DateTime.utc(captured.year, captured.month, captured.day).add(const Duration(days: 1));
...
Set<ReviewFlag> checkInstant(DueDate local, tz.Location location, DateTime referenceUtc) {
  if (!local.hasTime) return const {};
  final instants = wallTimeInstants(local, location);
  return {
    if (instants.length != 1) ReviewFlag.clockChange,
    if (instants.isNotEmpty && !instants.first.isAfter(referenceUtc)) ReviewFlag.timePassed,
  };
}
...
List<DateTime> wallTimeInstants(DueDate local, tz.Location location) {
  ...
  final offsets = <Duration>{
    for (final probe in [-1, 0, 1])
      location.timeZone(naive.add(Duration(days: probe)).millisecondsSinceEpoch).offset,
  };
  final instants = <DateTime>{};
  for (final offset in offsets) {
    final instant = naive.subtract(offset);
    final back = tz.TZDateTime.from(instant, location);
    if (back.hour == naive.hour && back.minute == naive.minute && back.day == naive.day) {
