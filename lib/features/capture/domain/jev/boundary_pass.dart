...
Map<String, JevQuestion> boundaryQuestions(List<TranscriptUnit> units) => {
  for (int i = 1; i < units.length; i++)
    boundaryKey(units[i]): NoulQuestion(
      'Look at unit ${units[i].id} and the unit immediately before it (${units[i - 1].id}). Immediately before unit ${units[i].id}, does a new, independently useful note or action begin, rather than a continuation, explanation, correction or reference belonging to the preceding thought? Answer yes when ${units[i].id} starts a separate thought or a separate action that makes sense filed on its own, even if it is about the same topic (two different errands are two actions). Answer no when ${units[i].id} continues or explains the previous words (for example "It went well"), corrects them (for example "Sorry, I meant Tuesday"), adds a detail such as a time, or only lists more objects for the same action (for example "and some eggs").',
      yes: 'A new, independent note or action starts at ${units[i].id}.',
      no: '${units[i].id} continues, explains or corrects the thought before it.',
    ),
};
...
Map<String, JevQuestion> lateCorrectionQuestions(List<TranscriptUnit> units) => {
  for (int i = 2; i < units.length; i++)
    lateCorrectionKey(units[i]): NoulQuestion(
      'Look at unit ${units[i].id}. Does ${units[i].id} change or correct a detail (such as a time, date, name or amount) of something said earlier than the unit immediately before it (${units[i - 1].id})? Answer no if ${units[i].id} only corrects ${units[i - 1].id}, or if it is new content of its own.',
