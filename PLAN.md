...
- [x] A22 Every merge to `main` builds an ad-hoc signed Capture.app, packs it into a DMG and, once the `hard-eng` check on that commit has passed, publishes it as a GitHub pre-release; pull requests build and pack the DMG without publishing. Owner request 2026-09-18. Proof: `.github/workflows/release.yml` passes actionlint and zizmor; its build and `hdiutil` steps run locally (114 MB app, 47 MB DMG, checksum VALID). The first remote release is recorded under Delivery.
...
Delivery target: PR
Delivery: Pending — `feature/capture-alpha` pushed to the owner's public repo `sgaabdu4/capture` through the pre-push check (14/14 passed); PR https://github.com/sgaabdu4/capture/pull/1 opened; its first CI run passed in 338 s. After merge, the Release workflow must publish the first DMG pre-release.
