...
Delivery target: PR
Delivery: Pending — `feature/capture-alpha` pushed to the owner's public repo `sgaabdu4/capture` through the pre-push check (14/14 passed); no PR opened, so CI has not run yet.
