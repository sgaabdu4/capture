...
- [ ] A10 Settings/onboarding: TypeSafe key and Notion token stored only in Keychain (legacy keychain), validated with `GET /v1/models` and `GET /v1/users/me`; Notion parent page chosen by URL/ID and access checked. Proof: settings tests with fake store (pending) + live validation in `test/live/capture_e2e_test.dart`, run by the owner 2026-09-18.
...
- [ ] A12 Approved save: capture page → items → audio upload (M4A, within the workspace upload limit) → mark Saved; per-step progress persisted in SQLite; retry resumes after the last confirmed step and never duplicates (lookup by stable IDs before re-creating). Proof: save-sequence tests with injected failures (pending) + live save and re-save without duplicates in `test/live/capture_e2e_test.dart`, run by the owner 2026-09-18.
