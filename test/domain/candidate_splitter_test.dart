...
List<String> excerpts(String text) =>
    splitCandidates(text).map((u) => u.span.excerpt).toList();
...
void main() {
  ...
  test('a short coordinated object is not a candidate', () {
    ...
    expect(excerpts('Buy milk, eggs and bread.'), [
      'Buy milk, eggs and bread.',
    ]);
  });

  test('two coordinated actions produce a candidate', () {
    expect(excerpts('Buy milk and book a haircut'), [
      'Buy milk',
      'and book a haircut',
    ]);
  });
  ...
  test('marker runs cut once at their first word', () {
    expect(
      excerpts(
        'I had a lovely evening with Sarah and also I thought of an app',
      ),
      ['I had a lovely evening with Sarah', 'and also I thought of an app'],
    );
  });
  ...
  test('unit ids and boundary kinds are ordered', () {
    final units = splitCandidates(
      'First thing here. Second thing and a third one',
    );
  ...
  test('coverageProblems reports missing and duplicated text', () {
    ...
    expect(coverageProblems(text, [SourceSpan.of(text, 0, 3)]), [
      'missing: "two three"',
    ]);
    expect(
      coverageProblems(text, [
        SourceSpan.of(text, 0, 7),
        SourceSpan.of(text, 4, 13),
      ]),
      ['duplicated: "two"'],
    );
    expect(
      coverageProblems(text, [
        const SourceSpan(0, 3, 'xyz'),
        SourceSpan.of(text, 3, 13),
      ]),
