import 'package:capture/domain/dates/date_candidates.dart';
import 'package:capture/domain/dates/date_resolver.dart';
...
FoundCandidates find(String text) =>
    findDateCandidates(text, SourceSpan.of(text, 0, text.length));
...
void main() {
  ...
  test('a period word resolves AM/PM', () {
    expect(
      resolve('Dinner tonight at 8').date,
      const DueDate(2026, 9, 17, hour: 20, minute: 0),
    );
    expect(
  ...
  test('relative durations use the capture time', () {
    ...
    expect(
      resolve('in half an hour').date,
      const DueDate(2026, 9, 17, hour: 20, minute: 39),
    );
  ...
  test('daylight-saving gap and overlap are flagged', () {
    ...
    final gap = checkInstant(
      const DueDate(2027, 3, 28, hour: 1, minute: 30),
      london,
      captured,
    );
    expect(gap, contains(ReviewFlag.clockChange));
    final overlap = wallTimeInstants(
      const DueDate(2026, 10, 25, hour: 1, minute: 30),
      london,
    );
    expect(overlap, hasLength(2));
    expect(
      reminderInstant(const DueDate(2026, 10, 25, hour: 1, minute: 30), london),
      isNull,
    );
    expect(
