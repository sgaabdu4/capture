import 'package:capture/domain/jev/boundary_pass.dart';
import 'package:capture/domain/jev/classification_pass.dart';
import 'package:capture/domain/jev/jev_protocol.dart';
import 'package:capture/domain/models.dart';
import 'package:capture/domain/proposal/edits.dart';
import 'package:capture/domain/proposal/proposal_builder.dart';
import 'package:capture/domain/source_span.dart';
import 'package:capture/domain/text/assembly.dart';
import 'package:capture/domain/text/candidate_splitter.dart';
import 'package:capture/domain/text/titles.dart';
...
void main() {
  ...
  test('batching keeps every question and repeats full state', () {
    final questions = {
      for (var i = 0; i < 400; i++)
        'q$i': NoulQuestion('Question $i ${'padding ' * 60}'),
    };
  ...
  test('negated, historical and recall thoughts do not become tasks', () {
    ...
    final answers = decodeResponse({
      'answers': {
        ..._thought('T1', 'Personal', task: 0.4, alert: 0.05),
        ..._thought('T2', 'Personal', task: 0.02),
